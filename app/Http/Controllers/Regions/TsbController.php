...
class TsbController extends Controller
{
    public function ShowPage($account)
    {
        ...
        if(!$region1)
        {
            ...
            $category =Category::select('category.*',DB::raw('(select count(*) from advert where advert.category = category.id and advert.city='.$region[0]['id'].') as count_sub'))
        ...
        if(!$region0)
        {
            ...
            $category =Category::select('category.*',DB::raw('(select count(*) from advert where advert.category = category.id and advert.region='.$region[0]['id'].') as count_sub'))
                ->where('category.id','<>','36')
                ->orderBy('title','ASC')
                ->get();
        }
        if(!$region2)
        {
            ...
            $category =Category::select('category.*',DB::raw('(select count(*) from advert where advert.category = category.id and advert.country='.$region[0]['id'].') as count_sub'))
                ->where('category.id','<>','36')
                ->orderBy('title','ASC')
                ->get();
        }
